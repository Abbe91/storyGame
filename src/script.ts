...
function showThequestion(theQuestionIndex: number){
  const theQuestion = theQuestions.find(theQuestion => theQuestion.id === theQuestionIndex)
...
function showOption(option: string){
  return option.requiredState == null || option.requiredState(state)

}
function selectOption(option: string){
 const nextQuestionId = option.nextQue
